...
function shouldBypassReadSandbox(event: ToolResultEvent): boolean {
  if (event.toolName !== "read") return false;

  const input = event.input as { path?: string; __pi_context_saver_skip?: unknown };
  if (input.__pi_context_saver_skip === true) return true;

  const rawPath = input.path ?? "";
  if (/\.(png|jpe?g|gif|webp|bmp|ico|avif|tiff?)$/i.test(rawPath)) return true;
  if (rawPath.toLowerCase().includes("screenshot") || rawPath.toLowerCase().includes("sandbox")) return true;

  return event.content.some((block) => block.type !== "text");
}
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("tool_result", async (event: ToolResultEvent, ctx: ExtensionContext) => {
    ...
    if (shouldBypassReadSandbox(event)) return;
